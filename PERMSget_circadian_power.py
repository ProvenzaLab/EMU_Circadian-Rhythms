...
PATH = "/mnt/labworlds/Provenza/EMU_Circadian-Rhythms/chunks_power_continuous_PERMS_new"

# Output folder: one CSV per patient
OUT = "/mnt/labworlds/Provenza/EMU_Circadian-Rhythms/circ_power_continuous_PERMS_new"

save_path = "/mnt/labworlds/Provenza/EMU_Circadian-Rhythms/all_electrodes_24Power_PERMS_new"
...
def compute_electrode_bands(file_name):
    ...
    for perm in range(100):
        df_perm = df[df["permutation"] == perm].copy()
        for band in F_NAMES:


            df_band = df_perm[df_perm["feature_name"] == band].copy()
            df_exponent = df_perm[df_perm["exponent"].notna()].copy()
...
Parallel(n_jobs=-1, verbose=0)(
    delayed(compute_electrode_bands)(file_name) for file_name in files
)
...
for folder in os.listdir(OUT):
    ...
    if dfs:
        ...
        out_path = os.path.join(save_path0, f"{folder}_ALL_SHUFFLED_ELECTRODES.csv")
