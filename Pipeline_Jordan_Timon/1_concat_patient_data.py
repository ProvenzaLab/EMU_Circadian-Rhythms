...
PATH_DATA = "data_250Hz_new"
PATH_electrodes = "/mnt/datalake/data/emu/"
patients = [f for f in os.listdir(PATH_DATA) if f.startswith("YF")]
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"meta_joined/{patient}_coords_ns3_mapped.csv"):
        ...
        if patient_sub != "YFU":
            ...
            df_elecsub = pd.read_csv("/mnt/projectworlds/EMU-18112/YFU_Datafile/IMG/YFU-electrodes_v2025+.csv")
    ...
    path_out_patient = f"chunks_out_fixed_new/{patient}/"
    if not os.path.exists(path_out_patient):
        os.makedirs(path_out_patient, exist_ok=True)
    #for npy_ in tqdm(npy_files):

    # using joblib
    #read_npy_file(npy_files[3])  # test run
    Parallel(n_jobs=40)(
        delayed(read_npy_file)(npy_) for npy_ in tqdm(npy_files)
    )

# run this using
# nohup python 1_concat_patient_data.py > concat_patient_data.log 2>&1 &
