...
def compute_patient_ch(file_name):
    ...
    if os.path.exists(f"chunks_power_continuous_new/{pat}_{ch}_power.csv"):
        return
    ...
    while start < len(data):
        ...
        print(fm.freqs.min(), fm.freqs.max(), len(fm.freqs))
        ...
        for f_range,f_name in zip(f_l,f_names):
            ...
            print(f_name, np.sum(idx), fm.freqs.min(), fm.freqs.max())
    ...
    df_nan = pd.DataFrame(s_)
    df_power.to_csv(f"chunks_power_continuous_new/{pat}_{ch}_power.csv",index=False)
    df_nan.to_csv(f"DataInfo/{pat}_{ch}_data_info.csv",index=False)
...
if __name__ == "__main__":
    ...
    Parallel(n_jobs = 40,verbose=0)(delayed(compute_patient_ch)(path_pass) for path_pass in files)
