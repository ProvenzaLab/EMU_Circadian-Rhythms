...
for subject in patients:
    ...
    files = list(Path(f"chunks_out_fixed/{subject}").rglob("*.npy"))
    ...
    def save_time(time_):
        ...
        np.save(os.path.join("foundation_model_prep", f"{subject}_{time_}_data.npy"), arr)
        #np.save(os.path.join("foundation_model_prep", f"{subject}_{time_}_coords.npy"), coord_df.values)
        # save as csv
        coord_df.to_csv(os.path.join("foundation_model_prep", f"{subject}_{time_}_coords.csv"), index=False)
